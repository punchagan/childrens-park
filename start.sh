...
elif [ `tail -1 nohup.out|grep -c -E "(WARNING - could not connect to server - aborting)|(Disconnected from server)|xmpp.protocol.SystemShutdown: (u'system-shutdown', '')"` -gt 0 ]; then
